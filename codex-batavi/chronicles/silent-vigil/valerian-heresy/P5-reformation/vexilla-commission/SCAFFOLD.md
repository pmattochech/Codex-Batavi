# Scaffold — *Vexilla Commission*

**Tree:** Valerian Heresy · **main serial**
**Saga #:** **23 of 25** · **Slug:** `vexilla-commission`
**Status:** **scaffold** — Prose not drafted — write from this file into `chapter.md`.
**Prose:** [chapter.md](chapter.md)

**Global locks:** [`../../SCAFFOLD.md`](../../SCAFFOLD.md) · [`../../PROSE-SCAFFOLD.md`](../../PROSE-SCAFFOLD.md) · Geography: [`castra-vetera-galactic-lock.md`](../../../../atlas-and-topography/castra-vetera-galactic-lock.md)

---

## Anchor

| Field | Lock |
|-------|------|
| **~M42** | ~087–088 *(timestamp sweep pending)* |
| **Quarter** | P5 — Reformation |
| **Theater** | Strategium / *Armourium* — anti-concentration reform |
| **POV** | Kadmos + Strategium staff |

## Thesis

**Vexilla I–IX** born **after** heresy — fragment the hammer; specialty → Legatus pipeline.

## Beat locks

- ~2,500 under one rebel nearly broke mesh
- Nine specialty Vexillationes
- Legati from squad lineages
- User pipeline detail TBD

## Scene spine (draft targets)

1. Commission debate — never again one host
2. Nine stencil lanes — specialty theaters named
3. Field 4 rule explained — future register
4. First Legati names **not** all Corbec generation
5. Close: vexilla commission sealed

## Irreversible fact (must file)

***Alabarda* vexilla I–IX** commissioned — civil war era **no cloth** ends here.

## Chronicle law

- Post-dates all field-4 certifications
- Anti-concentration lock

## Voice / craft

- Institutional reform — not victory parade

## Chain

- **Precedent scaffold:** [prima-obligation](../prima-obligation/SCAFFOLD.md)
- **Next scaffold:** [primaris-after-ash](../primaris-after-ash/SCAFFOLD.md)

## Depth target

~200–260 lines

## Open questions

- Nine specialty squad → Legatus mapping — user detail

