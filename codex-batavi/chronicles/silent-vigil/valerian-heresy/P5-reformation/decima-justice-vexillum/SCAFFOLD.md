# Scaffold — *Decima Justice Vexillum*

**Tree:** Valerian Heresy · **main serial**
**Saga #:** **21 of 25** · **Slug:** `decima-justice-vexillum`
**Status:** **draft** — Register-structured draft — may keep section headers or flatten at novel pass.
**Prose:** [chapter.md](chapter.md)

**Global locks:** [`../../SCAFFOLD.md`](../../SCAFFOLD.md) · [`../../PROSE-SCAFFOLD.md`](../../PROSE-SCAFFOLD.md) · Geography: [`castra-vetera-galactic-lock.md`](../../../../atlas-and-topography/castra-vetera-galactic-lock.md)

---

## Anchor

| Field | Lock |
|-------|------|
| **~M42** | ~086–087 *(timestamp sweep pending)* |
| **Quarter** | P5 — Reformation |
| **Theater** | Noviomagus *Armourium* / *Reclusiam* |
| **POV** | Varro / Kadmos register voice |

## Thesis

**Justice cloth** — sentence made heraldic; wolf transfixed by blade.

## Beat locks

- Not trophy skin — verdict
- *Decima* separate from I–IX
- Wolf feral not seal wolf
- Post-Malakor week

## Scene spine (draft targets)

1. Forge stencil — Kadmos
2. Varro oath alignment — no theater voice
3. Drusus margin — Cull cross-file
4. First bearer logged
5. Close: cloth legible across kill-zone

## Irreversible fact (must file)

***Decima* justice vexillum** commissioned — treason body cited.

## Chronicle law

- Register voice allowed
- Sympathy heresy — Varro correction

## Voice / craft

- Documentary craft in narrative frame

## Chain

- **Precedent scaffold:** [malakor-v](../malakor-v/SCAFFOLD.md)
- **Next scaffold:** [prima-obligation](../prima-obligation/SCAFFOLD.md)

## Depth target

~150–200 lines — draft on disk

## Open questions

- *(none — locks sufficient)*

## Retcon / pass notes

**051→~087** header; companion link to malakor-v paths updated.

