# Space Wolves & traitor-legion relations — worksheet (backlog / not canon until scenes ship)

**Goal:** Keep **diplomatic, aesthetic, and battlefield** stories with **Space Wolves** and with **Traitor / Heresy-imprint** foes **aligned** with [progenitor-classification.md](personae-command-index/doctrine-and-organs/progenitor-classification.md) — **Russ-line–adjacent** phenotype **without** open-record Fenris parentage; **Strand β** as **sealed** clinical language, **never** a public “Sons of Horus successor” label unless you write a leak arc.

**Law:** [`.cursorrules`](../.cursorrules) §0.2 · heraldry separation: [intro-and-heraldry.md](personae-command-index/intro-and-heraldry/intro-and-heraldry.md) · outsider beliefs: [`FOUNDING-OUTSIDER-KNOWLEDGE-PLAN.md`](FOUNDING-OUTSIDER-KNOWLEDGE-PLAN.md).

---

## 1. Non-negotiables (quick audit)

| Layer | Space Wolves | Traitor Legions / XVI echo |
|-------|--------------|-----------------------------|
| **Open Imperial / remembrancer** | May note **gifts**, **wolf helms**, **cultural parallel**; may **guess** SW-line seed — Chapter **does not confirm** | **No** “descended from XVI” as **Administratum** fact. Heraldry = **art history**, not gene receipt |
| **Battlefield banter** | Fenris warriors may call Batavi **cousins** or **wrong wolves** — **their** words, not omniscient narration as proof | Traitors may **feel** *Lupercal* shock (see Alaric dossier **terror math**) — **their** PTSD, not a tithe scroll |
| **Sealed *Apothecarion*** | Strand **α** = Russ-line–**compatible** scaffold (Cawl-era frame) | Strand **β** = **Mournival-pattern / treason-imprint sensitivity** — **not** same as “we are Sons of Horus” on a requisition form |

---

## 2. Space Wolves — relationship axes (fill as you write)

Use this so **alliance** stays **credible** without **collapsing** unknown founding.

| Axis | Canon hooks already in repo | Story levers / tensions |
|------|-----------------------------|-------------------------|
| **Material diplomacy** | *Morkai’s Bite* (Grimnar gift), **Bjorn** belt talisman, **Fenris**-forged relics on Alaric/Varro (see [dossier-alaric.md](personae-command-index/character-dossiers/triumvirate/dossier-alaric.md), [dossier-varro.md](personae-command-index/character-dossiers/triumvirate/dossier-varro.md)) | Gifts prove **respect**, not **gene-seed disclosure** |
| **Visual kinship** | *Fenris-pattern* / **wolf-skull** helms as **Noviomagus adaptations** | SW may read **kin**; Batavi read **equipment lineage** + **doctrine difference** (*Analytical Silence* vs howls) |
| **Biology** | Manuals still map lupine defects to **Fenris** as “loud ghost” ([chimera-pendulum.md](chronicles/03-projection-aurea-codex-omega/chimera-pendulum.md)) | Apothecary **friction**: Fenris baselines **fail** to explain *Das Erbe* fully |
| **The Thunderwolf line** | **Batav Wolf** / *Canis Batavorum* **≠** Fenris **Thunderwolf** ([GLOSSARY-EN.md](GLOSSARY-EN.md)) | Scene where a Wolf **mistakes** the pelt or the beast — **correct** them in-character |

**Chronicle touchpoints (non-exhaustive):** [gray-silence-of-fenris.md](chronicles/08-audit-diplomacy/gray-silence-of-fenris.md), [viggo-saga.md](chronicles/05-viggo-saga/viggo-saga.md), [diplomacy-aethelgard.md](chronicles/08-audit-diplomacy/diplomacy-aethelgard.md), [dragon-in-the-hearth-tushan.md](chronicles/08-audit-diplomacy/dragon-in-the-hearth-tushan.md) (compare: other Chapter visits).

---

## 3. Traitor Legions & Heresy-imprint — relationship axes

| Axis | Canon hooks | Writing guardrails |
|------|-------------|---------------------|
| **Tactical shock** | Alaric silhouette → **genetic memory hesitation** in Traitor veterans (*dossier-alaric* **terror math**) | Describe **effect**, not **gene confession** from Batavi POV in early eras |
| **Strand β** | *Blutrecht* / “judge” biology; **Mournival-pattern** internal tag | Only **Triumvirate / sealed clinical** POV after your timeline allows; no **line-troop** primarch laundry lists |
| **Blackshield / renegade** | Policy in [deathwatch-and-batavorum.md](political-relations/deathwatch-and-batavorum.md) | Do not **merge** blackshield fiction with “secretly XVI” without a deliberate arc |

---

## 4. Scene prompts

- **Fenris hall:** Logan (or successor) offers **mjod**; Batavi **accept** or **decline** on **culture**, not on admitting parentage.  
- **Joint deployment:** Space Wolf **howls**; Batavi **vox** stays **flat** — **friction** as **doctrine**, not insult.  
- **Traitormarine POV:** **Half-second** freeze seeing *Castra-Lupus* — **no** narrator sentence “proving” XVI descent.  
- **Inquisition:** Investigator conflates **wolf helm** with **Heresy** — Varro or *Strategium* **dismantle the logic** without handing them Strand β files.

---

## 5. Done for this backlog slice

- [ ] You have **updated** at least **one** existing chronicle note (inline comment or appendix) **or** drafted **one** new beat using §2–§4, **or**  
- [ ] You explicitly **defer** with a dated reason.

---

*Last reviewed: 2026-03-28.*
