# Insula Tenebrarum — isolation fence

**Status:** Authoritative for **new** atlas, origin, and curse-adjacent geography.  
**Does not** rewrite marsh-channel names or the Nine Phalanx map this pass. **Witness Ford** as the **point of the V** = working note; full atlas redraw **later**.  
**Charter:** [`castra-vetera-galactic-lock.md`](castra-vetera-galactic-lock.md)  
**Origin war:** [`the-reclamation.md`](../personae-command-index/events-and-chronologies/the-reclamation.md)  
**Curse claim line:** [`wolfs-curse-origin-and-den.md`](../personae-command-index/doctrine-and-organs/wolfs-curse-origin-and-den.md)

**Forbidden:** time dilation, ricochet rewind, “fast dome,” Faraday-cage metaphors that deny Warp inside the mesh.

---

## I. Mortals and Terra

| Item | Lock |
|------|------|
| **Castra mortals** | **Old Imperial remnant** — forgotten tithe, not a never-Crusaded wild people. |
| **Ghost tithe** | The tithe is **real**. Terra **cannot reach it** except when the **anomaly** opens. Paper continues. Hulls go out **~once per 20 years**. Between windows they **overproduce** and stockpile. |
| **How the Chapter got in** | **Mars** fires a **one-shot bore**. They enter. The hole **closes**. They **go dark** — no Imperium comms. |

---

## II. The fence (Men of Iron)

The **Men of Iron** built the fence. It is a **villain machine**, not a gift.

| Item | Lock |
|------|------|
| **What it is** | Realspace **and** Warp **reflector**. Not a Faraday cage. Not a time engine. |
| **Touch without a wound** | You are **gone**. No clever skimming of the skin. |
| **Inside the V** | **Warp is normal.** Marsh channels still work. The island is not a dead zone. |
| **Cicatrix** | Would swallow the micro-sector. It **hits the reflector and splits**. The mesh sits in a **V** of still-realspace. The cage **becomes a shield**. |

---

## III. Navel vortex (when the MoI are beaten)

The vortex is **not** a door the Men of Iron meant to open. It is **system failure** when they are **beaten**. It sits at the **geographic navel** (systems **face** it; not literal orbits).

| Item | Lock |
|------|------|
| **Cause** | MoI machine **fails** under defeat. |
| **Mechanicus** | Can only **stabilize** enough that the Warp explosion does **not** engulf the whole **subsector**. |
| **Job (after)** | The **only painful comms** toward the Imperium **and** a **Chaos invasion door**. |
| **Cost** | Isolation is not purity. The den-spirit kept **most** mortals clean; this door still feeds the tide. |

---

## IV. Pylon / anomaly (one opening per ~20 years)

A local **pylon** (Mechanicus **does not understand** it) can **briefly** open the fence. This is the **anomaly**.

They spend **most** of the Hold **closed**. Culture depth is cage-time, not traffic.

| Item | Lock |
|------|------|
| **Cadence** | **Able to open once per ~20 years.** Not months. Not a yearly tithe run. Across **~300** Hold years that is **~15** windows. |
| **What may leave** | **Hulls** jump to **safe realspace beside the Rift** (**Nihilus or Sanctus**) — tithe, proof, rare bodies. |
| **What may not** | **Comms** as a standing line. A jumped hull is not a vox to Terra. |
| **Tithe law** | Output is **always overproduced** (super-tithe, **≥15%** over quota as the floor). Stockpile between windows; dump a **fat** convoy when the anomaly opens. **Badab lesson:** they will **not** become **Lufgt Huron** — no withheld tithe, no “the center abandoned us so we keep the guns.” Isolation is never an excuse to short the Throne. |
| **Den law** | While the hull is **outside** the V, it is **outside the den**. Re-entry is mesh weather again. |

---

## V. Hull vs boots (curse and visitors)

**Same weather.** Entering **Insula Tenebrarum / the V** counts — a ship in the mesh is in the den. Terra-side of the fence is not.

- **Unraised** Batavi-pattern marine: curse **off** until **mesh entry**; then dump risk without raising.  
- **Visitor field:** begins at mesh entry (watched, not-kin, do not rule here).  
- **Raised Batavi** off-island (pylon jump, later Ford): still carry the curse.

Do not split “orbit vs rock” this pass. Both are inside if they are inside the V.

---

## VI. Ford (later atlas)

When a **standing wound** is allowed, **Witness Ford** is the working candidate for the **point of the V**. Do not redraw the three marsh channels until that pass.

---

*Last aligned: MoI reflector fence; V-shield at Cicatrix; navel vortex as MoI-defeat failure; anomaly **once per ~20 years**; super-tithe always on (Badab lesson); mesh entry = den weather.*
