"""Propose codex-batavi paths (dry-run only — never writes)."""
from __future__ import annotations

from typing import Any


CODEX_BESTIARY = "codex-batavi/biological-encyclopedia-bestiary"
...
def propose(world: dict[str, Any]) -> list[str]:
    ...
    paths = [
        f"{CODEX_BESTIARY}/generated/{slug}.md  (new Magos+literary merge candidate)",
        f"{CODEX_BESTIARY}/fauna-flora-named-specimens.md  (append rows for new named slots)",
    ]
    # Guess matrix by system name conventions
    sys_l = system.lower()
    if "bastion" in sys_l or "noviomagus" in sys_l or "central" in sys_l:
        paths.append(
            f"{CODEX_BESTIARY}/biogeographic-matrix-central-bastion.md  "
            f"(update/expand ### section for {slug})"
        )
    elif "crucible" in sys_l or "aethelgard" in sys_l or "incus" in sys_l:
        paths.append(
            f"{CODEX_BESTIARY}/biogeographic-matrix-crucible.md  "
            f"(update/expand ### section for {slug})"
        )
    elif "threshold" in sys_l or "vigil" in sys_l or "tempest" in sys_l:
        paths.append(
            f"{CODEX_BESTIARY}/biogeographic-matrix-threshold.md  "
            f"(update/expand ### section for {slug})"
        )
    else:
        paths.append(
            f"{CODEX_BESTIARY}/biogeographic-matrix-<system>.md  "
...
def format_report(world: dict[str, Any]) -> str:
    lines = [
        f"Codex propose (dry-run) for body `{world['meta']['slug']}`",
        "No files will be written under codex-batavi/.",
        "",
        "Suggested targets:",
    ]
