...
from lib import packs, pipeline, propose_codex, state  # noqa: E402
from lib.util import set_active_pack  # noqa: E402


def cmd_layers(_: argparse.Namespace) -> int:
    print("castra-biogen pipeline layers\n")
...
def cmd_propose_codex(args: argparse.Namespace) -> int:
    try:
        world = state.load_world(args.slug)
    except FileNotFoundError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    print(propose_codex.format_report(world))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="castra-biogen", description="40k system/biosphere generator")
    ...
    pc = sub.add_parser("propose-codex", help="Dry-run suggested codex-batavi paths")
    pc.add_argument("slug")
    pc.set_defaults(func=cmd_propose_codex)
