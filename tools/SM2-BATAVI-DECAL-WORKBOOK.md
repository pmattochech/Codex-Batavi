# Space Marine 2 — Batavi shoulder heraldry workbook

*Tooling — not in-universe canon.*

## Preferred path — Additional Chapters blank slots

**Active path (Astartes + AC):** [`sm2-batavi-ac-overlay/`](sm2-batavi-ac-overlay/) — fills blank **SH User 01** pauldrons (`d_shldr_user_texture_01*`). Shoulders only.

**Stack:** Astartes Overhaul → `sh0030_bridge…` → `sh0100_additional_chapters` → `sh0110_batavi_shoulders` → `zz_sh0120_batavi_user_materials` (see overlay README).

**Your next step:** `cd tools/sm2-batavi-ac-overlay && ./run convert` (or TexMipper on `tga/*.tga` → copy mips to `stage/pct/` → `./run pack`).

Author recipe lives in the AC download: `Your Own Pauldrons and Greaves.txt`. Full operator notes: [`sm2-batavi-ac-overlay/README.md`](sm2-batavi-ac-overlay/README.md).

---

## Alternate pipeline — Night Lords replace / new SSO slot

The sections below document the older [`sm2-batavi-decal/`](sm2-batavi-decal/) Path A / Path B toolkit (replace Night Lords textures or clone SSO for a true Armouring Hall entry). Prefer the AC blank-slot path above when available.
